...
static void canvas_update_proc(Layer *this_layer, GContext *ctx) {
  ...
  if (i <= 10){ // 10 min
    if (theme_buffer == 0){
      black_line_update(GPoint(30, 0), GPoint(30, 10), ctx); //5 min
    }else{
...
static void battery_handler(BatteryChargeState new_state) {
  ...
  if (new_state.is_charging){
    ...
    if (theme_buffer == 0){
      s_battery_status = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_RED_CHARGING);
    } else {
      s_battery_status = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_RED_20);
    }
    ...
    if (theme_buffer == 0){
      ...
      s_battery_status = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_RED_40);
    }
    ...
    if (theme_buffer == 0){
      ...
      s_battery_status = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_RED_60);
    }
    ...
    if (theme_buffer == 0){
      ...
      s_battery_status = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_RED_80);
    }
...
//       APP_LOG(APP_LOG_LEVEL_INFO, "Using OPTION 1");
      ...
      s_battery_status = gbitmap_create_with_resource(RESOURCE_ID_BATTERY_RED_100);
    }
  }
  bitmap_layer_set_bitmap(s_battery_layer, s_battery_status);
  bitmap_layer_set_compositing_mode(s_battery_layer, GCompOpSet);
...
static void update_theme(){
  if (theme_buffer == 1){
    ...
    text_layer_set_text_color(s_before_hour_1_layer, GColorBlack); // Before
    text_layer_set_text_color(s_after_hour_1_layer, GColorBlack); // After
  }
...
static void main_window_load(Window *window) {
  ...
  if (theme_buffer == 1){
    ...
    create_text_layer(s_background_layer, GColorRed, GColorClear, s_time_font_small, GTextAlignmentCenter);
    ...
    create_text_layer(s_before_hour_1_layer, GColorClear, GColorBlack, s_time_font, GTextAlignmentCenter);
    create_text_layer(s_after_hour_1_layer, GColorClear, GColorBlack, s_time_font, GTextAlignmentCenter);
